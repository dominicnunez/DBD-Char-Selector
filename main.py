...
def display_available_characters(team, available_characters):
    """Displays the available characters for exclusion or clearing."""
    print(f"\nAvailable {team} characters:")
...
def exclude_character():
    ...
    available_characters = [char for char in characters if char not in excluded]

    display_available_characters(team, available_characters)
        ...
        try:
            ...
            if exclusion_index in cancel():
                ...
                if 1 <= exclusion_index <= len(available_characters):
                    ...
                    confirmation = input(f"Are you really sure you want to remove {exclusion} from {team}s? ")
                    if confirmation.lower() in yes():
                    ...
                    print("Invalid number selection. Please enter a number that corresponds to one of the listed characters.")
        except ValueError:
...
def clear_exclusions():
    ...
    if not excluded_characters:
        ...
        display_available_characters(team, excluded_characters)
        ...
        while True:
            ...
            if clear_choice.isdigit():
                ...
                if clear_index > 0 and clear_index <= len(excluded_characters):
                    ...
                    confirmation = input(f"Are you sure you want to clear {character} from the {team} excluded list? ")
                    if confirmation.lower() in yes():
                ...
                confirmation = input(f"Are you sure you want to remove all characters from the {team} excluded list? ")
                if confirmation.lower() in yes():
                    character_selector.excluded_characters[team] = []
                    print(f"All characters have been cleared from the {team} excluded list.")
                else:
                    print("Clear exclusion operation canceled.")
                break
