...
def default_killers():
  return ["Trapper", "Wraith", "Hillbilly", "Nurse", "Huntress"]


def default_survivors():
  return ["Dwight", "Meg", "Claudette", "Jake", "Nea"]
...
def initialize_config(config_file):
    ...
    try:
        ...
        print("Invalid configuration file: 'Settings' section not found. Creating default config\n")
        create_config(config_file)
        with open(config_file) as f:
            config.read_file(f)
    ...
    killers = parse_list(config, 'killerList', default_killers)
    survivors = parse_list(config, 'survivorList', default_survivors)
